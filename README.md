...
- **Управление воспроизведением YandexMusicWatch** — команды "включи волну", "включи музыку", "выключи музыку", "включи любимую музыку", "следующий трек", "предыдущий трек" управляют плеером отдельного приложения YandexMusicWatch без необходимости его открывать.
...
системный фонарик, а сам системный «фонарик» на этих часах и так не
физический (LED), а имитация — заливка экрана белым цветом на максимальной
яркости. Раз это всё равно лишь экранный эффект, AssistVoice делает то же
...
Шесть зашитых команд — "включи волну", "включи музыку", "выключи музыку",
"включи любимую музыку", "следующий трек", "предыдущий трек" — управляют
воспроизведением в отдельном приложении YandexMusicWatch, неофициальном
клиенте Яндекс Музыки для часов. Как и команды фонарика, они
зашиты напрямую в грамматику распознавателя, не являются слотами, не
настраиваются и не отображаются в UI, всегда активны.

Управление реализовано через стандартный протокол Android `MediaController`
(AndroidX Media3) — тот же, которым пользуются Android Auto или кнопка
воспроизведения на Bluetooth-гарнитуре: AssistVoice подключается напрямую к
сервису воспроизведения YandexMusicWatch по фиксированному имени компонента,
не открывая его интерфейс. Соединение устанавливается заново на каждую
команду, а не держится постоянно, — если YandexMusicWatch в этот момент не
запущен, команда запускает его в фоне.
